...
class DarisServices:
    ...
    def connect(self, args):
        # print self.server + " " + str(self.port) + " " + self.domain + " " +
        # self.username + " " +  self.password
        v = args.split('/')
        #print >> sys.stderr, v
        if(len(v) != 3):
            return self.jsonError('Wrong argument string')
        ret = daris.DarisUtils.daris_logon(
            self.server, self.port, v[0], v[1], v[2])
        if(ret == -1):
    ...
    # print xmlstr
        ...
        projects = []
        query = "asset.query :where \"cid starts with '" + repoid + \
            "' and model='om.pssd.project'\" :action get-meta :size infinity"
        xmlstr = daris.DarisUtils.daris_runcommand(query, sid)
        ...
        for project in projects:
            ...
            pro_json['title'] = 'project: ' + project['cid'] + \
    ...
    def getMembers(self, sid, cid):
        ...
        for member in members:
            ...
            if(member['type'] != 'dataset'):
                member_json['title'] = member['type'] + ': ' + member['cid'] + \
