#!/usr/bin/env python
# obj2stl.py Convert Alias OBJ mesh file to STL
# $Header$
# $Id$
# $Date$
#
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(usage='''obj2stl.py -i <zipfile>|<directory>|OBJ file  ''',
                                     description='''Create Websurfer model.tok file from obj files.  Load a zip or directory (-i <file>) that contains OBJ  files.    ''' )
    parser.add_argument(
        '-i', '--input', help='''Input directory or *.obj file name. Directory must contain obj files, or a subdirectory with obj files, or a zip file with similar properties. ''', required=True)
    parser.add_argument(
        '-o', '--output', help='''Output directory or file name. Must be a directory with write access or filename with extension *.stl. If not included the input dir/file will be used.''', )
...
#        logging.error(' Folder \'' + args.input + '\' does not exist.')
    ...
    if not os.path.isdir(args.output):
        print 'Folder \'' + args.output + '\' does not exist.'
